...
# Adapated from:
# https://opensource.apple.com/source/xnu/xnu-6153.141.1/tools/remote_build.sh
...
if [ "$REMOTEBUILD" = "$SPECIALREMOTEBUILD" ]; then
    ...
    dest_dir="mkdir -p $REMOTEBUILDPATH/BUILD/{obj,sym,dst}";
    # shellcheck disable=SC2029
    ssh "$REMOTEBUILD" "$dest_dir" || die "Could not make remote build directory."
